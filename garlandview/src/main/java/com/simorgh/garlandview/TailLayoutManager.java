...
public class TailLayoutManager extends RecyclerView.LayoutManager
    ...
    public int scrollHorizontallyBy(int dx, RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        for (int i = 0, cnt = getChildCount(); i < cnt; i++) {
            final TailItem vh = (TailItem) mRecyclerView.getChildViewHolder(Objects.requireNonNull(getChildAt(i)));
            if (vh.isScrolling()) {
                return 0;
            }
        }
    ...
    private void fillRight1(int anchorPos, RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        final int anchorViewRight;
        final View anchorView = mViewCache.get(anchorPos);
        if (anchorView != null) {
            anchorViewRight = getDecoratedRight(anchorView);
        } else {
            anchorViewRight = mItemStart + viewWidth;
        }
    ...
    private void fillLeft1(int anchorPos, RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        int viewRight = mItemStart + viewWidth;
        int pos = anchorPos;
